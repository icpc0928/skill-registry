...
| [api-conventions](./skills/api-conventions/) | 寫 API endpoint 時自動套用團隊規範（URL 命名、回傳格式、status code、input validation） | 1.0.0 | `api` `backend` `conventions` `restful` |
...
### Claude Code Slash Command（推薦）

部分 Skill 設計為 Claude Code 的 `/指令`，複製 `SKILL.md` 到專案的 `.claude/commands/` 即可：

```bash
# 在專案根目錄執行
mkdir -p .claude/commands
curl -o .claude/commands/commit.md \
  https://raw.githubusercontent.com/icpc0928/skill-registry/main/skills/commit/SKILL.md
```

之後在 Claude Code 輸入 `/commit` 即可執行。

### 方式 1：直接給 AI 讀

```
請讀取 https://raw.githubusercontent.com/icpc0928/skill-registry/main/skills/qa-framework/SKILL.md
然後為我的專案生成 QA 測試框架。
```
...
```bash
git clone https://github.com/icpc0928/skill-registry.git
```
...
```bash
# 下載特定 skill 資料夾
svn export https://github.com/icpc0928/skill-registry/trunk/skills/qa-framework
```
